...
const Select = memo(({ personalData, salaryData }: { personalData: MemberDataTypes; salaryData: SalaryDataTypes[] }) => {
  ...
  const handleClickRow = (year: number) => {
    isOpenDetailYear.includes(year) ? setIsOpenDetailYear(isOpenDetailYear.filter(item => item !== year)) : setIsOpenDetailYear([...isOpenDetailYear, year]);
  };
            ...
            {data?.data.map((data, index) => {
              ...
              return (
                <React.Fragment key={index}>
                  <TableRow className="cursor-pointer h-[53px] bg-primary-foreground" onClick={() => handleClickRow(data.year)}>
