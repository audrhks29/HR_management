...
const MonthPicker = memo(({ isMonthPicker, setIsMonthPicker, selectedMonth, setSelectedMonth }: {
  ...
  setSelectedMonth: React.Dispatch<React.SetStateAction<{
    year: string;
    month: string;
  }>>
}) => {
  ...
  return (
    <div className='absolute top-8 right-1/2 translate-x-1/2 z-10'>
      <Button variant="outline" onClick={() => setIsMonthPicker(!isMonthPicker)}>
        <Calendar className='mr-3' />{selectedMonth.year}년 {selectedMonth.month}월
      </Button>

      {isMonthPicker && <Card className='w-[500px] pt-6 absolute top-12 right-1/2 translate-x-1/2'>
        <CardContent className='text-center grid grid-cols-2 gap-6'>
          ...
          <Button
            className='col-span-2 w-[150px] m-auto'
            onClick={handleButtonClick}>조회</Button>
        </CardContent>
      </Card>}
    </div>
  );
